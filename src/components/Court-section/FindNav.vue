<template>
  <nav class="navBar">
    ...
    <div class="nav__search" v-if="navShow">
    ...
    <div :class="[loggedIn ? 'nav__user__active' : 'nav__user']">
      <router-link :to="{ name: 'user_booking' }" v-if="loggedIn">
        <v-badge
          :content="courtCount"
          :value="courtCount"
          color="green"
      ...
      <router-link :to="{ name: 'log_in' }">
        <h1 class="user__login" v-if="!loggedIn">Login</h1>
      </router-link>
      <router-link :to="{ name: 'sign_up_first' }" v-if="!loggedIn">
        <v-btn rounded>Sign up</v-btn>
      </router-link>
      <v-btn rounded @click="logout" v-if="loggedIn"> Logout </v-btn>
    </div>
  </nav>
</template>
